...
public class JavaStatementVisitor extends ASTVisitor{
	 ...
	 * @return
	 */
	private StatementContext evaluateComplexMetric(ASTNode node)
	{
		...
		statement.setCognitiveComplexity(fragmentVisitor.getMetric());
		return statement;
	}
	...
	public boolean visit(VariableDeclarationStatement vdStatement) {
		StatementContext statement = evaluateComplexMetric(vdStatement);
	...
	public boolean visit(ExpressionStatement expStatement) {
		StatementContext statement = evaluateComplexMetric(expStatement);
		statementList.add(statement);
		return false;//无须检索子节点。
	}
...
/*	@Override
	...
	public boolean visit(AssertStatement assertStatement) {
		StatementContext statement = evaluateComplexMetric(assertStatement);
		statementList.add(statement);
	...
	public boolean visit(ConstructorInvocation ciStatement) {
		StatementContext statement = evaluateComplexMetric(ciStatement);
		statementList.add(statement);
		return false;//视作一条完整语句，不检索子节点。
	}
	...
	public boolean visit(SuperConstructorInvocation superStatement) {
		StatementContext statement = evaluateComplexMetric(superStatement);
		statementList.add(statement);
		return false;//视作一条完整语句，不检索子节点。
	}
	...
	public boolean visit(BreakStatement breakStatement) {
		StatementContext statement = evaluateComplexMetric(breakStatement);
		statementList.add(statement);
		return false;//视作一条完整语句，不检索子节点。
	}
	...
	public boolean visit(ContinueStatement continueStatement) {
		StatementContext statement = evaluateComplexMetric(continueStatement);
		statementList.add(statement);
		return false;//视作一条完整语句，不检索子节点。
	}
	...
	public boolean visit(ReturnStatement returnStatement) {
		StatementContext statement = evaluateComplexMetric(returnStatement);
		statement.addSpeccialComplex(MetricCognitiveComplexity.ReturnStmt);
		statementList.add(statement);
		return false;//视作一条完整语句，不检索子节点。
	}
	...
	public boolean visit(ForStatement forStatement) {
		...
		for(  Object obj : forInits )
		{
			...
			StatementContext statement = evaluateComplexMetric(express);
	...
	public boolean visit(EnhancedForStatement enforStatement) {
		...
		StatementContext statement = evaluateComplexMetric(svd);
		enforContext.enlargeToComplexStatement(statement);
		//Expression部分。
		Expression	express = enforStatement.getExpression();
		statement = evaluateComplexMetric(express);
		enforContext.enlargeToComplexStatement(statement);
		//增强型for语句的两部分组成一个StatementContext，加入到语句列表。
		enforContext.addSpeccialComplex(MetricCognitiveComplexity.LOOP);
	...
	public boolean visit(DoStatement doStatement) {
		 ...
		 *以后再修改。 		
		 */
		StatementContext statement = evaluateComplexMetric(doStatement.getExpression());
		statement.addSpeccialComplex(MetricCognitiveComplexity.LOOP);
		statementList.add(statement);
		return true; //必须true,搜索其子节点。
	}	
	...
	public boolean visit(WhileStatement whileStatement) {
		StatementContext statement = evaluateComplexMetric(whileStatement.getExpression());
		statement.addSpeccialComplex(MetricCognitiveComplexity.LOOP);
	...
	public boolean visit(IfStatement ifStatement) {
		StatementContext statement = evaluateComplexMetric(ifStatement.getExpression());
		statement.addSpeccialComplex(MetricCognitiveComplexity.IfElse);
	...
	public boolean visit(SwitchStatement switchStatement) {
		StatementContext statement = evaluateComplexMetric(switchStatement.getExpression());
		statement.addSpeccialComplex(MetricCognitiveComplexity.SwitchStmt);
	...
	public boolean visit(SwitchCase caseStatement) {
		...
		StatementContext statement = evaluateComplexMetric(express);
		statement.addSpeccialComplex(MetricCognitiveComplexity.CaseStmt);
	...
	public boolean visit(SynchronizedStatement synchStatement) {
		StatementContext statement = evaluateComplexMetric(synchStatement.getExpression());
		statement.addSpeccialComplex(MetricCognitiveComplexity.SynchronizedStmt);
	...
	public boolean visit(ThrowStatement throwStatement) {
		StatementContext statement = evaluateComplexMetric(throwStatement);
		statement.addSpeccialComplex(MetricCognitiveComplexity.ThrowStmt);
		statementList.add(statement);
 ...
 */
	...
	public boolean visit(TryStatement tryStatement) {
		...
		{ // try [ ( Resources ) ]作为一个整体。
			...
			resourceContext.addSpeccialComplex(MetricCognitiveComplexity.CatchCause); //try以及每个CatchClause复杂度+3
		...
		*/
		...
		for( Object obj: catchCauchLst)
		{
			...
			StatementContext statement = evaluateComplexMetric(svdCatch);
			statement.addSpeccialComplex(MetricCognitiveComplexity.CatchCause);
